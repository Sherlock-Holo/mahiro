//! io-uring register buffer wrapper, this is inspired from
//! https://github.com/tokio-rs/io-uring/blob/0a8d5d16d2248f2b68aa8ab54b502524363cafb6/io-uring-test/src/tests/register_buf_ring.rs
...
impl InnerBufRing {
    ...
    fn buf_ring_push(&self, bid: Bid) {
        ...
        self.local_tail.store(old_tail + 1, Ordering::Release);
...
struct FixedSizeBufRing {
    ...
    share_inner_buf_ring: Arc<InnerBufRing>,
}

impl FixedSizeBufRing {
    fn new(buf_ring: InnerBufRing) -> Self {
        FixedSizeBufRing {
            share_inner_buf_ring: Arc::new(buf_ring),
        }
    }
    ...
    where
        ...
        C: cqueue::EntryMarker,
    {
        self.share_inner_buf_ring.register(ring)
    }
    ...
    where
        ...
        C: cqueue::EntryMarker,
    {
        self.share_inner_buf_ring.unregister(ring)
    }

    /// Returns the buffer group id.
    fn bgid(&self) -> Bgid {
        self.share_inner_buf_ring.bgid()
    }

    /// Returns the buffer the uring interface picked from the buf_ring for the completion result
    /// represented by the res and flags.
    fn get_buf(&self, res: u32, flags: u32) -> io::Result<GBuf> {
        self.share_inner_buf_ring.get_buf(self.clone(), res, flags)
    }
}
...
impl Debug for GBuf {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("GBuf")
            .field("bgid", &self.buf_group.bgid())
            .field("bid", &self.bid)
            .field("len", &self.len)
            .field("cap", &self.buf_group.share_inner_buf_ring.buf_capacity())
            .finish()
    }
}

impl GBuf {
    fn new(buf_group: FixedSizeBufRing, bid: Bid, len: usize) -> Self {
        assert!(len <= buf_group.share_inner_buf_ring.buf_len);
    ...
    #[allow(dead_code)]
    fn cap(&self) -> usize {
        self.buf_group.share_inner_buf_ring.buf_capacity()
    }

    /// Return a byte slice reference.
    fn as_slice(&self) -> &[u8] {
        let p = self.buf_group.share_inner_buf_ring.stable_ptr(self.bid);
...
impl Drop for GBuf {
    fn drop(&mut self) {
        ...
        unsafe { self.buf_group.share_inner_buf_ring.dropping_bid(self.bid) };
    }
}
